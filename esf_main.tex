...
\subsubsection{Wiring / additional circuitry} \label{sec:shutdown_wiring}
	\Fref{fig:shutdown_circuit} shows the wiring of the shutdown circuit.
...
TODO
	...
	\begin{table}[H]
	    ...
	    \begin{tabular}{|l|l|}
	        ...
	        Maximum charging voltage & 4.15 V (Per Cell) \\ \hline
	        Maximum charging current & 6 A \\ \hline
	...
	This CE-Listed charger supplies constant current at 6A until cell voltages reach 4.15V. Then, constant voltage is supplied at 4.15V per cell, summing to 298.8 volts, until ??? TODO determine this behavior - we can program it???. The charging power lines ???TODO: Talk to Alex about fusing???. 
	
	The accumulator will only be charged on the charging cart. It will be able to support the full weight of the accumulator and only move when a dead man's switch is activated.
	
	The accumulator cart will contain a specialized shutdown and monitoring circuit that allows the accumulator to be charged through the main pack connector. The AMS will be active during charging and have the ability to open the AIRS and stop charging in the event of dangerous battery conditions. This shutdown circuit will contain a clearly labeled emergency stop push button with a minimum diameter of 25mm which opens the AIRS.  
	
	???Interlock and IMD TODO ???
