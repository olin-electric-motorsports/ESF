...
The Sensata Resettable crash sensor (6-11g version) will trigger due to an impact that decelerates the vehicle between 6-11g.
	...
	\begin{table}[H]
	    ...
	    Supply voltage & \hl{15 VDC} \\ \hline
...
\hl{The energy meter enclosure has been designed to accommodate the Formula Student Germany energy meter for compliance with Formula North rules, and will have a set of smaller bus bars to mount to the 2017 Formula SAE Electric energy meter. See} \fref{fig:energy_meter_both} \hl{to see both energy meters superimposed.}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Energy_MeterwithBoth}
\caption{\hl{Rendering of the two energy meters superimposed upon each other in the energy meter enclosure. See figure {\ref{fig:energy_meter_mounting_render}} for a render of the enclosure in the vehicle.}}
\label{fig:energy_meter_both}
\end{figure}
...
\Fref{fig:energy_meter_both} shows the internals of the energy meter enclosure from another angle, also including the housing. Internal covers are removed to show bus bar geometry. Bus bars will be wrapped in Kapton polyimide tape with a protective heat-shrink sleeve.
